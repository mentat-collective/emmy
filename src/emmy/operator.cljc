...
(deftype Operator [o arity name context]
  ...
  (zero-like [this]
    (if-let [z-fn (:zero-like context)]
      (z-fn this)
      (Operator. v/zero-like arity 'zero context)))

  (one-like [this]
    (if-let [one-fn (:one-like context)]
      (one-fn this)
      (Operator. core/identity arity 'identity context)))
  ...
  #?@(:clj
      [IObj
       (meta [_] (meta o))
       (withMeta [_ meta] (Operator. (with-meta o meta) arity name context))
      ...
      [IMeta
       ...
       IWithMeta
       (-with-meta [_ meta] (Operator. (-with-meta o meta) arity name context))
...
(defn ^:no-doc with-context
  ...
  (if (operator? op)
    (let [op ^Operator op]
      (->Operator (.-o op) (.-arity op) (.-name op) ctx))
    (u/illegal (str "non-operator supplied: " op))))
