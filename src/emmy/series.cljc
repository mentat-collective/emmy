...
(ns emmy.series
  ...
  (:require [emmy.expression];; for the effect of the defmethod of zero? for Literals
            [emmy.function :as f]
            [emmy.generic :as g]
...
(deftype Series [xs m]
  ...
  ad/IPerturbed
  (perturbed? [_] false)
  (replace-tag [s old new] (fmap #(ad/replace-tag % old new) s))
  (extract-tangent [s tag] (fmap #(ad/extract-tangent % tag) s))
